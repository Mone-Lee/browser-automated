...
function runOneShot(input: SkillTriggerInput): TestResult {
  const agent = new BrowserAgent();
  const start = Date.now();

  try {
    const step = executeDeterministicScenario(agent, input.url, input.instruction);
...
export class BrowserE2ESkillService {
  ...
  async runOneShotInstruction(input: SkillTriggerInput): Promise<SkillTriggerResult> {
    ...
    const execution = runOneShot(input);
    ...
    return {
      ...
      execution,
      generated,
      guidance: `✓ 测试通过，已生成 Playwright 测试代码：${generated.filePath}`,
    };
  }
